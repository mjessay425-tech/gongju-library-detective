...
function buildFireworkFX(className, big=false){
  ...
  const bursts=big ? [
    [12,20,1.25],[31,30,1.05],[52,17,1.45],[73,28,1.15],[90,18,1.05],
    [20,48,1.0],[43,43,1.28],[66,49,1.12],[84,45,1.0],
    [9,72,.95],[30,67,1.15],[54,73,1.35],[77,68,1.12],[94,75,.9]
  ] : [
    [18,28,1.0],[50,22,1.12],[82,30,.92],[28,50,1.08],[68,50,1.15],[12,72,.9],[50,68,1.2],[88,70,1.0]
  ];
  bursts.forEach(([x,y,scale],bi)=>{
    const b=document.createElement('div'); b.className='firework';
    b.style.left=x+'%'; b.style.top=y+'%'; b.style.setProperty('--scale',scale);
    b.style.setProperty('--delay',(bi*(big?.12:.18))+'s');
    const rays=big?24:18;
    for(let i=0;i<rays;i++){
      const p=document.createElement('i');
      p.style.setProperty('--angle',(i*(360/rays))+'deg');
      p.style.setProperty('--dist',(big?(52+(i%4)*10):(38+(i%3)*8))+'px');
      p.style.setProperty('--pdelay',((i%6)*(big?.025:.035))+'s');
      b.appendChild(p);
    }
    fx.appendChild(b);
  });
  const count=big?58:28;
  for(let i=0;i<count;i++){
    const c=document.createElement('span'); c.className='celebrate-confetti';
    c.textContent=['✦','✧','★','✺','●','◆','✿'][i%7];
    c.style.left=(2+(i*17)%96)+'%'; c.style.top=(4+(i*29)%82)+'%';
    c.style.setProperty('--delay',((i%12)*(big?.09:.17))+'s');
    c.style.setProperty('--rot',((i*47)%360)+'deg');
    fx.appendChild(c);
  }
  if(big){
    for(let i=0;i<12;i++){
      const s=document.createElement('span'); s.className='celebrate-star';
      s.textContent=['✦','★','✧'][i%3];
      s.style.left=(8+(i*31)%84)+'%'; s.style.top=(12+(i*19)%72)+'%';
      s.style.setProperty('--delay',((i%7)*.14)+'s'); s.style.setProperty('--rot',((i*37)%360)+'deg');
      fx.appendChild(s);
    }
  }
  return fx;
...
function addTypingDialogue(s,stage){
  ...
  box.className='dialogue-overlay';
  box.style.left=pct(d.bx,853); box.style.top=pct(d.by,1844);
  box.style.width=pct(d.bw,853); box.style.height=pct(d.bh,1844);
  const bg=document.createElement('img');
  bg.src=d.bubble; bg.alt=''; bg.className='dialogue-bubble';
  const text=document.createElement('div');
  text.className='typing-text';
  text.style.left=pct(d.tx-d.bx,d.bw);
  text.style.top=pct(d.ty-d.by,d.bh);
  text.style.width=pct(d.tw,d.bw);
  text.style.height=pct(d.th,d.bh);
  text.style.fontSize=(d.size||27)+'px';
  box.appendChild(bg); box.appendChild(text); s.appendChild(box);
